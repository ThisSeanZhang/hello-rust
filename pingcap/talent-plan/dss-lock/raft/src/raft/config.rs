...
impl Config {
    ...
    pub fn new_with(server_num: usize, unreliable: bool, snapshot: bool) -> Config {
        init_logger();
        let net = labrpc::Network::new();
        net.set_reliable(!unreliable);
        net.set_long_delays(true);
        let storage = Storage {
            logs: vec![HashMap::new(); server_num],
            max_index: 0,
            max_index0: 0,
        };
        let mut saved = vec![];
        ...
        for _ in 0..server_num {
            endnames.push(vec![String::new(); server_num].into_boxed_slice());
            saved.push(Arc::new(SimplePersister::new()));
        }
        let mut cfg = Config {
            net,
            n: server_num,
            rafts: Arc::new(Mutex::new(vec![None; server_num].into_boxed_slice())),
            connected: vec![true; server_num].into_boxed_slice(),
